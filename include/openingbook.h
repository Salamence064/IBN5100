...
// The default buffer size for opening book. This is equivalent to 512KB + 256KB + 8 bytes.
// This way we can store up to uint16_max positions in our buffer at any time alongside the number of positions.
#define BUFFER_SIZE 524288 + 262144 + 8
...
namespace IBN5100 {
    ...
    class OpeningBook {
        private:
            ...
            static constexpr void writeToFile(std::string const &filename, char* buffer, size_t bufferSize, uint16_t numPositions);
        ...
        public:
            ...
            // Save the position to the opening book.
            void save(Position const &pos, int score);
